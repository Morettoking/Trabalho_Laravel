...
@section('admin-content')
  <form method="POST" action="{{ route('admin.vehicles.store') }}" class="card">
    @csrf
    ...
    <div class="grid gap-4 md:grid-cols-3">
      ...
      <div>
        ...
        <select name="car_model_id" id="car_model_id" class="select" required>
          <option value="">Selecione</option>
          @foreach ($models as $m)
            <option value="{{ $m->id }}" data-brand="{{ $m->brand_id }}" @selected(old('car_model_id')==$m->id)>
              {{ $m->brand->name }} - {{ $m->name }}
            </option>
          @endforeach
        </select>
    ...
    <div class="grid gap-4 md:grid-cols-3 mt-4">
      <div>
        ...
        <input name="title" value="{{ old('title') }}" class="input">
        @error('title') <div class="mt-1 text-sm text-red-600">{{ $message }}</div> @enderror
      </div>
      <div>
        <label class="label">Ano</label>
        <input type="number" name="year" value="{{ old('year') }}" class="input" required>
        @error('year') <div class="mt-1 text-sm text-red-600">{{ $message }}</div> @enderror
      </div>
      <div>
        <label class="label">Quilometragem (km)</label>
        <input type="number" name="mileage_km" value="{{ old('mileage_km') }}" class="input" required>
    ...
    <div class="grid gap-4 md:grid-cols-2 mt-4">
      <div>
        ...
        <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="input" required>
        @error('price') <div class="mt-1 text-sm text-red-600">{{ $message }}</div> @enderror
      </div>
      <div>
        <label class="label">Foto Principal (URL)</label>
        <input name="main_photo_url" value="{{ old('main_photo_url') }}" class="input" placeholder="https://...">
    ...
    <div class="mt-6">
      ...
      <div id="photos" class="mt-2 space-y-2">
        @php
          $oldPhotos = old('photo_urls', ['', '', '']); // 3 campos iniciais
        @endphp
